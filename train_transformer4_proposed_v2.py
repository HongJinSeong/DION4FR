...
if __name__ == '__main__':
    ...
    SAVE_WEIGHT_DIR = './outpainting/checkpoints/HKPU_B/'
    SAVE_LOG_DIR = './outpainting/logs_all/HKPU_B/'
    ...
    def get_args():
        ...
        parser.add_argument('--epochs', type=int, help='number of epoches', default=1200)
    ...
    tds = glob(join('datasets', 't2_kj'), '*', True)
    ...
    transformations = transforms.Compose(
        [torchvision.transforms.RandomResizedCrop((192, 192), scale=(0.8, 1.2), ratio=(0.75, 1.3333333333333333), ),
         CenterCrop(192), ToTensor(), Normalize(mean, std)])
    ...
    train_data = dataset_norm(root=args.train_data_dir, transforms=transformations, imgSize=192, inputsize=128,
                              imglist=ttls)
    train_loader = DataLoader(train_data, batch_size=args.train_batch_size, shuffle=True, num_workers=8)
    ...
    valid_data = dataset_norm(root=args.train_data_dir, transforms=transformations, imgSize=192, inputsize=128,
                              imglist=tvls)
    valid_loader = DataLoader(valid_data, batch_size=args.train_batch_size, shuffle=True, num_workers=8)
