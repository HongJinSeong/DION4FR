...
from .swin_transformer4_cross_attn import SwinTransformer4 as swin_cross_attn
from .swin_transformer4_cross_attn_ResB import SwinTransformer4 as swin_cross_attn_RESB  ## Proposed
from .swin_transformer4_cross_attn_ResB_v2 import SwinTransformer4 as swin_cross_attn_RESB_v2  ## Proposed


from .swin_transformer4_ResB import SwinTransformer4 as swin_resB
...
def build_model(config):
    ...
    elif model_type == 'swin_cross_attn':
        model = swin_cross_attn(pre_step=config['pre_step'],
                                img_size=config['IMG_SIZE'],
                                patch_size=config['SWIN.PATCH_SIZE'],
                                in_chans=config['SWIN.IN_CHANS'],
                                embed_dim=config['SWIN.EMBED_DIM'],
                                ...
                                patch_norm=config['SWIN.PATCH_NORM'],
                                use_checkpoint=config['TRAIN.USE_CHECKPOINT'])
    elif model_type == 'swin_cross_attn_ResB':
        model = swin_cross_attn_RESB(pre_step=config['pre_step'],
                                img_size=config['IMG_SIZE'],
                                patch_size=config['SWIN.PATCH_SIZE'],
                                in_chans=config['SWIN.IN_CHANS'],
                                embed_dim=config['SWIN.EMBED_DIM'],
                                ...
                                use_checkpoint=config['TRAIN.USE_CHECKPOINT'])
    ### RESNET ablation
    ### don't mind cross just mistype
    elif model_type == 'swin_cross_ResB':
        model = swin_resB(pre_step=config['pre_step'],
                                img_size=config['IMG_SIZE'],
                                patch_size=config['SWIN.PATCH_SIZE'],
                                in_chans=config['SWIN.IN_CHANS'],
                                embed_dim=config['SWIN.EMBED_DIM'],
